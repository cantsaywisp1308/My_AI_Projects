...
class train_model:
    ...
    def test(self, test_data):
        try:
            ...
            y_predict = self.mul_lr.predict([test_predict])
            return  y_predict
        except:
            print("Factors of finding Personality are not entered")
...
def prediction_result(top, aplcnt_name, cv_path, personality_values):
    ...
    personaility = model.test(personality_values)
    ...
    Label(result, text= str("Predicted Personality: " + personaility).title(), foreground='black', bg='white',anchor='w').pack(fill=BOTH)
    quitBtn = Button(result, text='Exit', command= lambda :result.destroy()).pack(fill=BOTH)
...
def predict_person():
    ...
    lab = Label(top, text="Personality Prediction", foreground='red', bg='black', font=titleFont)

    #Job Form
    job_list = ('Select a job', '101-Developer at Google', '102 - Chef at Gordon Ramsay','103 - Professor at MIT')
    job = StringVar(top)
    job.set(job_list[0])
    ...
    l5 = Label(top, text="Enjoy New Experience or thing (Openess)", foreground='white', bg='black').place(x=70, y=250)
    ...
    extroversion = Entry(top)
    extroversion.insert(0, '1 - 10')
    extroversion.place(x=450, y=370, width=160)

    submitBtn = Button(top, padx=2, pady=0, text="Submit", bd=0,foreground='white', bg='red', font=(12))
    submitBtn.config(command=lambda : prediction_result(top, sName, loc, (gender.get(), age.get(), openness.get(), neuroticism.get(), conscientiousness.get(),
                                                                          agreeableness.get(), extroversion.get())))
...
def OpenFile(b4):
    global loc;
    name = filedialog.askopenfilename(initialdir="D:\\", filetypes=(("Document","*.docx"),("PDF","*.pdf"),("All files","*")), title="Choose a file.")
    try:
...
if __name__ == '__main__':
    ...
    b2 = Button(root, padx=4, pady=4, width=30, text='Predict Personality', bg='black', foreground='white', bd=1, font=homeBtnFont, command= predict_person()).place(relx=0.5, rely=0.5, anchor=CENTER)
    root.mainloop()